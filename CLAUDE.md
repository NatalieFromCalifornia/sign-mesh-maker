...
- **3MF geometry must be indexed — one vertex per position, shared by every triangle meeting there.** The format describes a surface through those indices and a slicer reads the topology it is given rather than inferring one, so writing three fresh vertices per triangle declares a mesh where no two triangles are joined: every edge belongs to one face, which is every edge non-manifold, and the model needs repairing before it will slice. **STL hides this completely** — it is a soup of loose triangles by definition, so anything reading one welds by position; that is why "STL fine, 3MF broken" is the signature. Vertices are keyed on the coordinates *as written*, not as held in memory, so two that the file cannot tell apart share an index instead of sitting a rounding error away from each other, and a triangle whose corners round together is dropped. Indexing also cut a real sign's vertex count from 155,988 to 26,080.
  - **Vertices are written to a hundredth of a micrometre**, which is about topology rather than printing. Indexing keys on the coordinates as written, so two boundaries passing within a rounding step of each other get stitched into one vertex — a pinch, and an edge with four faces on it. Real geometry lands under half a micrometre apart often enough that micrometre precision produced exactly that.
  - Auditing the in-memory `partsFromGroup` output cannot see this, because that check welds by position — exactly what the format does not do. Any manifoldness test has to parse the written file.
- **A stepped 3MF exports as height bands, not per-colour parts.** `buildHeightBands` slices the sign into one solid per step — band `k` is every layer reaching above it, so its exposed top is exactly layer `k`'s region — and each band becomes a part on its own extruder. That is what Orca's height-range painting produces: the filament changes between bands rather than between regions, which is the only way to print a stepped sign on a single extruder, and the walls stripe through the colours beneath them. **Flat mode is untouched** — every colour is at one height, so there are no bands, and its per-colour parts are what a multi-colour printer wants.
  - Deliberately *not* implemented as triangle painting. Orca stores that in `paint_color` on each `<triangle>` as a subdivision tree serialized to a bitstream — undocumented, and silently ignored when wrong. Bands reach the same result through the per-part extruder binding that already works. A reference export was decoded far enough to confirm the shape of it: the default extruder is unpainted, `"8"` was extruder 2 and `"0C"` extruder 3, and wall triangles crossing a band carry long subdivision strings.
...
- **Fills that differ only by rounding are folded together on upload.** Real artwork routinely carries a dozen fills where the eye sees three — a shape recoloured a shade off, a flattened gradient, a round trip through another tool — and each one would otherwise be a filament change and another step of height for a difference nobody can see. `clusterSimilarColors` returns an *assigned colour* per layer, so this is an ordinary merge (§5.4): it shows in the list as "N merged" and needs no separate mechanism. The largest region names its cluster, so the printed colour is one the artwork actually uses rather than an average that appears nowhere in it.
  - `SIMILAR_COLOR_DISTANCE` is 10, and the margin either side is wide. In a sign that read as three colours but carried twelve fills, every near-duplicate was within 5.01 of its neighbour and nothing else was closer than 93.99. The nearest genuine distinction in any test artwork is a cream background against white lettering at 19.82.
  - **Reset returns to that folded state, not to the raw parse** — `baseAssigned` holds it — because a dozen fills the eye reads as three is not a state to put someone back into. The Reset button appears once anything differs from it.
...
- **Every exported part must be a closed solid, and only the 3MF will tell you.** A plain STL puts every colour in one solid a slicer heals without comment; a 3MF hands each colour over as its own mesh, so an unclosed one slices into missing letters and floating-region warnings. Two things keep them closed, both in `buildMesh`:
  - **A deterministic sub-micron nudge before triangulation** (`breakTriangulationTies`). Earcut bridges each hole to the outer ring by casting a ray and gets it wrong when rings are *exactly* collinear — and cutting a line of text out of a background gives every glyph-hole its baseline on the same horizontal line. The cap then spans across several holes instead of following their edges, so cap and walls disagree and the solid is left open. The offset is hashed from the coordinates, not from a vertex's index, so a point shared by two layers moves the same way in both and touching walls stay touching. Measured on a real sign: 192 open edges to 0.
  - **Regions below `MIN_PRINTABLE_MM2` are dropped.** Boolean operations leave slivers of a thousandth of a square millimetre carrying a few hundred self-crossing points — unprintable, and impossible to triangulate into anything closed. `shapesFromTree` additionally runs every ring through `Clipper.CleanPolygon`, which removes the near-duplicate points a cut deposits along a shared edge.
  - **What reaches the extruder must be a well-formed polygon set** — disjoint regions, every hole inside the outline that owns it. Most of the pipeline gets that free because the clipping returns it, but two paths do not: the topmost layer is never cut against anything, and merging concatenates the shapes of every layer folded into it, which can overlap. `normalizeShapes` covers both. Separately, `insetShapes` offsets a layer's rings *in one operation* rather than one at a time — insetting can split a region in two, and the holes then belong to whichever piece contains them; the old ring-by-ring version handed every hole to every piece, putting holes outside their outline.
  - `buildMesh.test.ts` pins this with a fixture of glyphs sharing a baseline. Both watertightness tests fail without the nudge, including the one for an ordinary sign — this was not an exotic case.
...
- **Strokes are printed, by becoming the region they paint.** A printed sign has no strokes — every colour is a solid region of some thickness — so `strokeToShapes` offsets each stroked subpath by half its width through Clipper (`etClosedLine` for a closed one, which yields the ring with its hole; `etOpenButt` otherwise) and adds the result as a region in the stroke's own colour, after that element's fill, which is the order SVG paints them. Skipping strokes lost a sign's entire border with nothing to say why: the file was otherwise complete and every fill in it came through. A stroke-only SVG is therefore printable now, so the "nothing paintable" error only fires when there is genuinely no fill *and* no stroke.
  - Caveat worth knowing: three's `SVGLoader` transforms path points but **not** `strokeWidth`, so a stroked path under a scaling transform would come out the wrong width. No test artwork does that; if a sign ever shows a border of the wrong weight, that is the first thing to check.
...
```
upload (SVG)                          fills and strokes grouped by colour, one layer each, document order
